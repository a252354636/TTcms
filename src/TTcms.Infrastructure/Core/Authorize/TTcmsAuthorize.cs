...
{
    public abstract class TTcmsAuthorize : ITTcmsAuthorize
    {
        protected TTcmsAuthorizeModel CreateModel(HttpActionContext actionContext, string name)
        {
            var attributes = actionContext.ActionDescriptor.ControllerDescriptor.GetCustomAttributes<TTcmsApiAuthorizeAttribute>(true);
            ...
            return new TTcmsAuthorizeModel
            {
        ...
        protected TTcmsAuthorizeModel CreateModel(AuthorizationContext filterContext, string name)
        {
            var attributes = filterContext.ActionDescriptor.ControllerDescriptor.GetCustomAttributes(typeof(TTcmsMvcAuthorizeAttribute), true);
            if (attributes == null || attributes.Length <= 0)
            {
                throw new AuthorizationException();
            }
            var attribute = attributes[0] as TTcmsMvcAuthorizeAttribute;
        ...
        /// <returns></returns>
        protected abstract bool IsAuthorized(TTcmsAuthorizeModel model);
